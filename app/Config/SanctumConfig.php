...
class SanctumConfig implements Arrayable
{
    /**
     * @var string[]
     */
    ...
    {
        $config = new self();
        $config->stateful(explode(',', env('SANCTUM_STATEFUL_DOMAINS', sprintf('%s%s', 'localhost,localhost:3000,127.0.0.1,127.0.0.1:8000,::1', \Laravel\Sanctum\Sanctum::currentApplicationUrlWithPort()))));
        $config->guard([
            'web',
        ]);
        $config->expiration(\null);
